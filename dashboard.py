...
import streamlit as st  # python freamwork
import plotly.express as px  # generates the charts
import pandas as pd  # for data handling
import os  # navigates some file
import warnings
warnings.filterwarnings('ignore')

st.set_page_config(page_title="superstore!",
                   page_icon=":bar_chart:", layout="wide")
st.title(" :bar_chart: Dropout Student Analysis")
st.markdown(
...
if fl is not None:
    ...
    os.chdir(r"C:\xampp\htdocs\php\Hackathon")
...
st.sidebar.header("choose your filter: ")  # open a small sidebar on left side
...
# area=st.sidebar.multiselect("pick your area",df4["Area"].unique())

# filter the data based on rigion, state and city
if not region and not state and not city:
    ...
    filtered_df = df3[df["Region"].isin(
...
with cl1:
    with st.expander("Category_ViewData"):
        st.write(category_df.style.background_gradient(cmap="Dark2"))
